...
import { View, Platform, Text, CheckBox, AsyncStorage,Alert, TouchableOpacity} from 'react-native';
...
export default function Pergunta14() {
 ...
 const [isDisable3, setIsDisable3] = useState(false);
  ...
  useEffect(() => {
    ...
    if(isSelected1){
      setResposta1('Sempre que mexo em algo que outros também mexem')
    }else{ setResposta1('')}
    if(isSelected2){
      setResposta2('Quando penso em botar as mãos no rosto')
    }else{ setResposta2('')}
    if(isSelected3){
      setResposta3('Assim que chego em casa')
    }else{ setResposta3('')}
    if(isSelected4){
      setResposta4('Sempre que vou comer ou vou ao banheiro')
    }else{ setResposta4('')}
...
_storeData = async () => {
   ...
   try{
  await AsyncStorage.setItem(keys.questionario.Q14, JSON.stringify(res))
  const save = await AsyncStorage.getItem(keys.questionario.Q14)
  if(!save){
  ...
  return (
    <View style={styles.container}>
     ...
     <Text style={styles.titles} >Você lava as mãos com frequência?</Text>
     ...
     {os == 'ios' ?
        ...
        <Text >Sempre que mexo em algo que outros também mexem</Text>
      </View>
      ...
      {os == 'ios' ?
       ...
       <Text >Quando penso em botar as mãos no rosto</Text>
      </View>
      ...
      {os == 'ios' ?
       ...
       <Text >Assim que chego em casa</Text>
      </View>
      ...
      {os == 'ios' ?
       ...
       <Text >Sempre que vou comer ou vou ao banheiro</Text>
      </View>
      <View style={styles.alternatives}>
      {os == 'ios' ?
          <CheckBoxIOS
