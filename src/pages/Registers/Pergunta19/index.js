...
export default function Pergunta19() {
 ...
 const [isDisable8, setIsDisable8] = useState(false);
  ...
  useEffect(() => {
    ...
    if (isSelected1 == false  && isSelected2 ==false && isSelected3 ==false && isSelected4 ==false
      ...
      setResposta1('Acima de 7 cômodos') 
    }
...
_storeData = async () => {
   const respostas ={
     questao19_comodos_casa:{
       resposta:resposta1     
     }
   }
   try{
  await AsyncStorage.setItem(keys.questionario.Q19, JSON.stringify(respostas))
  const save = await AsyncStorage.getItem(keys.questionario.Q19)
  if(!save){
  ...
  return (
    <View style={styles.container}>
     ...
     <Text style={styles.titles} >Quantos cômodos tem na casa em que esta morando atualmente ? </Text>
